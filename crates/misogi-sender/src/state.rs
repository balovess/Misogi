...
use misogi_core::integrity::{IntegrityConfig, SessionManager};
// ABAC integration imports (Task 6.28)
use misogi_core::abac::{AbacEngine, ApprovalExecutor, AttributeResolver};
...
pub struct AppState {
    ...
    // =======================================================================
    /// CDR Engine v2 pipeline for advanced document sanitization.
    ///
    /// When `Some(...)`, this pipeline is used instead of legacy direct sanitizer
    /// calls for document processing. The v2 pipeline provides:
    /// - Stage-based processing architecture
    /// - Policy-driven decision making
    /// - Comprehensive audit trail generation
    /// - Support for custom sanitization stages
    ///
    /// `None` when CDR v2 is disabled (falls back to legacy sanitizers).
    ///
    /// **Configuration**: Set via `cdr_v2_enabled = true` in `[cdr_strategies]` TOML section.
    pub cdr_v2_pipeline: Option<Arc<CdrPipeline>>,
    ...
    ///
    /// When `Some(config)`, the tunnel task will wrap each chunk in an
    /// [`IntegrityEnvelope`] and perform per-chunk verification with
    /// automatic repair on corruption detection.
    /// When `None` (default), integrity verification is disabled and
    /// chunks are transmitted without cryptographic envelopes.
    ///
    ...
    /// Used by [`execute_transfer_integrity`](crate::tunnel_task::execute_transfer_integrity)
    /// to track transfer progress and enable resume after interruption.
    ///
    /// # Thread Safety
    ...
    // =======================================================================
    // ABAC Integration Layer (Task 6.28)
    // =======================================================================
    /// Attribute-Based Access Control (ABAC) policy evaluation engine.
    ///
    /// When enabled, evaluates access requests against policy rules before
    /// allowing file transfer operations. Produces structured decisions with
    /// obligations (approval requirements, MFA, justification, etc.).
    ///
    /// `None` when ABAC is disabled (default for backward compatibility).
    /// When `Some(...)`, all transfer requests are evaluated against the
    /// configured policy rules before proceeding.
    pub abac_engine: Option<Arc<AbacEngine>>,

    /// Attribute resolver for normalizing access request parameters.
    ///
    /// Converts raw request context (user identity, resource metadata,
    /// environmental context) into a unified `HashMap<String, AbacValue>`
    /// suitable for policy condition evaluation.
    ///
    /// Always present when `abac_engine` is enabled; `None` otherwise.
    pub attribute_resolver: Option<Arc<AttributeResolver>>,

    /// Approval executor for fulfilling ABAC obligation requirements.
    ///
    /// Manages the lifecycle of approval requests generated by policy
    /// obligations (`Obligation::RequireApproval`). Handles approver
    /// selection, timeout, auto-complete, and escalation.
    ///
    /// `None` when ABAC is disabled or when no approval templates are configured.
    pub approval_executor: Option<Arc<ApprovalExecutor>>,

    // =======================================================================
    // Multi-Tier Relay Integration (Task 6.25)
    // =======================================================================
    /// Integrated multi-tier relay mesh for cross-network file forwarding.
    ///
    /// When `Some(mesh)`, the sender can forward files through a multi-hop
    /// relay topology instead of direct point-to-point transfer. The mesh
    /// encapsulates:
    ...
    /// When enabled, the sender acts as an edge node in a relay mesh,
    /// forwarding files through intermediate proxy/hub nodes to reach
    ...
    #[allow(dead_code)]
    pub relay_mesh: Option<Arc<RelayMesh>>,
}
...
impl AppState {
    ...
    pub fn from_config(config: &SenderConfig) -> Arc<Self> {
        ...
        // --- 8. CDR Engine v2 Pipeline (Task 6.26) ---
        // Initialize CDR v2 pipeline when enabled in configuration.
        // Falls back to None (legacy sanitizers) when disabled.
        let cdr_v2_pipeline = if config.cdr_v2_enabled {
            let policy = CdrPolicy::default();
            let pipeline = CdrPipeline::new(policy);
            // Note: Stages are added dynamically based on file type during processing.
            // See upload_engine::sanitize_file_with_cdr_v2() for stage registration.
            Some(Arc::new(pipeline))
        } else {
            None
        };

        // --- 9. Legacy components (unchanged) ---
        ...
        // --- 10. Self-Healing Transport Integrity Layer (Task 6.27) ---
        // Initialize integrity subsystem when enabled in configuration.
        // When enabled, creates SessionManager for checkpoint persistence.
        // Falls back to None when disabled (backward compatible).
        let (integrity_config, session_manager) = if config.integrity_enabled {
            ...
            (Some(cfg), Some(Arc::new(session_mgr)))
        } else {
            (None, None)
        };

        // --- 11. ABAC Integration Layer (Task 6.28) ---
        // Initialize ABAC components when enabled in configuration.
        // ABAC provides attribute-based access control for transfer operations.
        // Falls back to None when disabled (backward compatible).
        let (abac_engine, attribute_resolver, approval_executor) =
            if config.abac_enabled {
                // Initialize attribute resolver with TTL-based caching
                let resolver = Arc::new(AttributeResolver::new(
                    config.abac_attribute_cache_ttl_secs,
                ));

                // Initialize ABAC engine with policy rules from configuration
                // Note: Policy rules should be loaded from config file or database.
                // For now, we create an empty engine that will use default-deny.
                let engine = Arc::new(AbacEngine::new(
                    vec![], // Empty rules: will use default_effect
                    misogi_core::abac::policy::PolicyEffect::Deny, // Default deny
                    config.abac_decision_cache_ttl_secs,
                ));

                // Initialize approval executor if approval templates are configured
                let executor = if !config.abac_approval_templates.is_empty() {
                    Some(Arc::new(ApprovalExecutor::new(
                        config.abac_approval_templates.clone(),
                    )))
                } else {
                    None
                };

                (Some(engine), Some(resolver), executor)
            } else {
                (None, None, None)
            };

        // --- 12. Multi-Tier Relay Mesh (Task 6.25) ---
        // Initialize relay mesh when relay mode is enabled in configuration.
        // The mesh integrates forwarder, node manager, and route planner.
        // Falls back to None when disabled (backward compatible).
        //
        // Note: Full relay configuration (topology, nodes, edges) should be
        // loaded from a dedicated [relay] TOML section. For now, we initialize
        // to None as the configuration schema for relay is not yet defined.
        // Future: Check config.relay_enabled and build topology from config.
        let relay_mesh: Option<Arc<RelayMesh>> = None;
    ...
    pub fn new(config: SenderConfig) -> Self {
        ...
        Self {
            ...
            cdr_v2_pipeline: None, // Legacy constructor does not enable CDR v2
            integrity_config: None, // Legacy constructor does not enable integrity
            session_manager: None,  // Legacy constructor does not enable integrity
            abac_engine: None,      // Legacy constructor does not enable ABAC
            attribute_resolver: None,
            approval_executor: None,
            relay_mesh: None,       // Legacy constructor does not enable relay
        }
    }
