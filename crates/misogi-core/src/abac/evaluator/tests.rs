...
fn test_eq_string_match() {
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("Role", AbacValue::String("admin".into()))])
        )
        .unwrap()
    );
}
...
fn test_eq_string_no_match() {
    ...
    assert!(
        !ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("Role", AbacValue::String("guest".into()))])
        )
        .unwrap()
    );
}
...
fn test_eq_integer() {
    let c = cond(
        AbacAttribute::ClearanceLevel(0),
        ConditionOperator::Eq,
        AbacValue::Integer(5),
    );
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("ClearanceLevel", AbacValue::Integer(5))])
        )
        .unwrap()
    );
}
...
fn test_eq_boolean() {
    let c = cond(
        AbacAttribute::MfaVerified(false),
        ConditionOperator::Eq,
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("MfaVerified", AbacValue::Boolean(true))])
        )
        .unwrap()
    );
}
...
fn test_in_list_contains() {
    let c = cond(
        ...
        AbacValue::List(vec![
            AbacValue::String("admin".into()),
            AbacValue::String("operator".into()),
        ]),
    );
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("Role", AbacValue::String("operator".into()))])
        )
        .unwrap()
    );
}
...
fn test_in_list_not_contains() {
    ...
    assert!(
        !ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("Role", AbacValue::String("auditor".into()))])
        )
        .unwrap()
    );
}
...
fn test_gt_integer_greater() {
    let c = cond(
        AbacAttribute::FileSizeBytes(0),
        ConditionOperator::Gt,
        AbacValue::Integer(1000),
    );
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("FileSizeBytes", AbacValue::Integer(5000))])
        )
        .unwrap()
    );
}
...
fn test_gt_integer_not_greater() {
    let c = cond(
        AbacAttribute::FileSizeBytes(0),
        ConditionOperator::Gt,
        AbacValue::Integer(5000),
    );
    assert!(
        !ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("FileSizeBytes", AbacValue::Integer(1000))])
        )
        .unwrap()
    );
}
...
fn test_lt_integer_less() {
    let c = cond(
        ...
        AbacValue::Integer(10_000_000),
    );
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("FileSizeBytes", AbacValue::Integer(1024))])
        )
        .unwrap()
    );
}
...
fn test_gt_type_mismatch_boolean() {
    ...
    let r = ConditionEvaluator::evaluate_condition(
        &c,
        &am(vec![("MfaVerified", AbacValue::Boolean(true))]),
    );
...
fn test_regex_match() {
    let c = cond(
        AbacAttribute::FileType(String::new()),
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            ...
            &am(vec![(
                "FileType",
                AbacValue::String("application/pdf".into())
            )])
        )
        .unwrap()
    );
...
fn test_regex_invalid_pattern_error() {
    ...
    let r = ConditionEvaluator::evaluate_condition(
        &c,
        &am(vec![("FileType", AbacValue::String("test".into()))]),
    );
...
fn test_regex_type_mismatch_non_string_attr() {
    ...
    let r = ConditionEvaluator::evaluate_condition(
        &c,
        &am(vec![("FileSizeBytes", AbacValue::Integer(1234))]),
    );
    assert!(matches!(r.unwrap_err(), EvalError::TypeMismatch { .. }));
}
...
fn test_ip_in_range_cidr_match() {
    let c = cond(
        AbacAttribute::IpAddress(String::new()),
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            ...
            &am(vec![(
                "IpAddress",
                AbacValue::String("10.100.50.23".into())
            )])
        )
        .unwrap()
    );
...
fn test_ip_in_range_cidr_no_match() {
    ...
    assert!(
        !ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("IpAddress", AbacValue::String("10.0.0.1".into()))])
        )
        .unwrap()
    );
}
...
fn test_ip_in_range_slash_24() {
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            ...
            &am(vec![(
                "IpAddress",
                AbacValue::String("192.168.1.100".into())
            )])
        )
        .unwrap()
    );
    ...
    assert!(
        !ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("IpAddress", AbacValue::String("192.168.2.1".into()))])
        )
        .unwrap()
    );
}
...
fn test_ip_in_range_slash_32_exact() {
    ...
    assert!(
        ConditionEvaluator::evaluate_condition(
            ...
            &am(vec![(
                "IpAddress",
                AbacValue::String("203.0.113.42".into())
            )])
        )
        .unwrap()
    );
...
fn test_attribute_not_found() {
    let c = cond(
        AbacAttribute::UserId(String::new()),
        ConditionOperator::Eq,
        AbacValue::String("anyone".into()),
    );
...
fn test_custom_attribute_key_lookup() {
    let c = cond(
        AbacAttribute::Custom {
            key: "custom_score".into(),
            value: AbacValue::Float(0.5),
        },
        ConditionOperator::Gt,
        AbacValue::Float(0.9),
    );
    assert!(
        ConditionEvaluator::evaluate_condition(
            &c,
            &am(vec![("custom_score", AbacValue::Float(0.95))])
        )
        .unwrap()
    );
}
